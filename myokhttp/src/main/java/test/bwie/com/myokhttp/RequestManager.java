...
public class RequestManager {
    ...
    public RequestManager(Context context, TextView tv) {
        //初始化OkHttpClient
    ...
    public static RequestManager getInstance(Context context,TextView tv) {
        RequestManager inst = mInstance;
        if (inst == null) {
            synchronized (RequestManager.class) {
                inst = mInstance;
                if (inst == null) {
                    inst = new RequestManager(context.getApplicationContext(),tv);
                    mInstance = inst;
                }
            }
        }
    ...
    private void requestPostBySynWithForm(String actionUrl, HashMap<String, String> paramsMap) {
        try {
            ...
            okHttpHandler.post(new Runnable() {
                ...
                public void run() {
                    try {
                        mTextView.setText(response.body().string() +" zzz");
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
